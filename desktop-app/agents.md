...
1.  **CORS Errors in Renderer**: Supabase queries or image loads hitting CORS limits.
    *   *Solution*: Execute the API call in the Main process using Electron `net.fetch`, or verify that RLS / Bucket permissions in the Supabase control panel are set to public-read.
2.  **Missing DB Schema Relational Triggers**: Insertions failing due to missing parent album rows.
    *   *Solution*: Ensure the parent album is validated and upserted in the database (via `supabase-insert-album` IPC handler) before inserting dependent photo metadata.
3.  **App Package Bundling Failures**: AWS SDK or Supabase JS client issues during `npm run build`.
    *   *Solution*: Verify these libraries are imported using dynamic imports (`await import(...)`) inside IPC handlers or marked as external in the `electron.vite.config.ts` setup.
